WhatsApp pode usar diferentes métodos de proteção de login: app autenticador, aprovação no app oficial, SMS, passkeys ou confirmação da conta. Adicione a conta ao TOTP somente quando WhatsApp mostrar um QR code ou uma chave manual para um app autenticador.
...
Digite em WhatsApp o código de 6 dígitos do TOTP.

### 7. Salve os códigos de recuperação

Se WhatsApp oferecer códigos de recuperação, guarde-os em segurança. Se houver apenas SMS, aprovação push, passkeys ou app oficial, use esse método separadamente.
