GOV.UK One Login can use different sign-in protection methods: an authenticator app, approval in the official app, SMS, passkeys, or account confirmation. Add the account to TOTP only when GOV.UK One Login shows a QR code or manual key for an authenticator app.
...
If GOV.UK One Login offers recovery codes, keep them somewhere safe. If only SMS, push approval, passkeys, or an official app are available, use that method separately.
